...
fmt = unlines [
  ...
  , "         a(i,j) = (a(i%s,j%s) + a(i%s,j%s) + a(i%s,j%s) + a(i%s,j%s))/4"
  , "            end do"
  , "     end do"
  , "  end do"
  , "  call output_ppm(6, a)"
...
fileName i = "jacobi/f/jacobi" ++ show i ++ ".f90"

main =
  mapM_ (\ (n, [a,b,c,d,e,f,g,h]) -> writeFile (fileName n) (printf fmt a b c d e f g h)) (zip [1..] (sequence (replicate 8 choices)))
