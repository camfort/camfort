...
spec = do
  describe "units-synth" $ do
    ...
    it "synthesises units for array_func.f90 using array_spec module" $
       unitsSynthReportWithMod ["array_spec.f90"] "array_func.f90" expectedArrayFuncOutput
...
expectedArrayFuncOutput :: String
expectedArrayFuncOutput = unlines
  [ "!= unit(k m**3) :: array_f"
  , "function array_f()"
  , ""
  , "    use array_spec"
