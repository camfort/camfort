  ...
  describe "Model spec" $ do
    describe "ioCompare" $ do
      let reg1 =
            ...
            \/
            Base [ Interval 0 1 True, Interval 0 2 False] :: Region 2
      let reg2 = Base [ Interval 0 2 True, Interval 0 2 False ] :: Region 2
      ...
      let reg3 =
            reg2 \/ Base [ Interval 0 3 False, Interval 0 0 True ] :: Region 2
      res <- runIO $ ioCompare reg3 reg2
      it "compares greater regions" $
        res `shouldBe` GT

      let reg4 = reg1 \/ Base [ InfiniteInterval, InfiniteInterval ] :: Region 2
