...
main = do putStrLn introMessage 
          ...
          if (length args == 4) then
             let (func:(dir:(excludes:(outdir:_)))) = args
             in case (lookup func (analyses ++ refactorings)) of 
                  Just (fun, _) -> fun dir excludes outdir
                  Nothing ->
                      case func of 
...
doAnalysis :: (Program A -> Program Annotation) -> Directory -> String -> IO ()
...
-- A simple, sample transformation
